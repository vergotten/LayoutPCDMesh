"""
This script provides utility functions for processing 3D mesh data and plane equations.

The functions include:

- `isFourPointsInSamePlane(p0, p1, p2, p3, error)`: Checks if four points are in the same plane within a certain error range.
- `get_normal(quad_vert, center)`: Calculates the normal vector of a plane defined by four points (vertices of a quadrilateral).
- `rectangle(quad_vert, center)`: Converts a quadrilateral into a rectangle by calculating its center, size, and orientation.
- `get_center(verts)`: Calculates the center of a set of points.
- `transform(scan_name, mesh_vertices)`: Transforms the vertices of a 3D mesh according to the axis alignment matrix of a given scan.
...
SCANNET_DATASET_DIR = config['SCANNET_DATASET_DIR']

from models.dump_helper import dump_pc


def isFourPointsInSamePlane(p0, p1, p2, p3, error):
    """Check if four points are in the same plane within a certain error range.

    Args:
        p0, p1, p2, p3 (numpy.ndarray): Four points to check.
        error (float): The error range.

    Returns:
        bool: True if the four points are in the same plane, False otherwise.
    """
    s1 = p1 - p0
    s2 = p2 - p0
    s3 = p3 - p0
...
def transform(scan_name, mesh_vertices):
    """Transform the mesh vertices according to the axis alignment matrix.

    Args:
        scan_name (str): The name of the scan.
        mesh_vertices (numpy.ndarray): The mesh vertices to transform.

    Returns:
        numpy.ndarray: The transformed mesh vertices.
    """
    meta_file = os.path.join(DATASET_BASE_DIR, 'scans_transform', scan_name, scan_name + '.txt')
...
def get_quads(scan_name):
    """Get the quads of a scan.

        Args:
            scan_name (str): The name of the scan.

        Returns:
            tuple: A tuple containing the rectangles, total quad number, and horizontal quads.
        """
    with open(os.path.join(DATASET_BASE_DIR, 'scannet_planes', scan_name + '.json'), 'r') as quad_file:
...
def get_quads_eval(scan_name):
    """Get the quads of a scan for evaluation.

        Args:
            scan_name (str): The name of the scan.

        Returns:
            tuple: A tuple containing the rectangles and horizontal quads.
        """
    with open(os.path.join(DATASET_BASE_DIR, 'scannet_planes', scan_name + '.json'), 'r') as quad_file:
        plane_dict = json.load(quad_file)
