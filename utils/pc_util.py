...
def write_oriented_bbox_with_normal(center, normal, size, out_filename, colors=None):
    """Export oriented (with normal vectors) scene bbox to meshes

    Args:
        center: Nx3, ndarray
        normal: Nx2, ndarray
        size: Nx2, ndarray
        out_filename (_type_): _description_
    ...
    import torch
    cos_theta = torch.cosine_similarity(torch.tensor(normal),torch.tensor([[0,1]]), dim=-1)
    heading_angle = torch.arccos(cos_theta)
    cos_theta1 = torch.cosine_similarity(torch.tensor(normal),torch.tensor([1,0]), dim=-1)
    if cos_theta1>0:
    ...
    write_oriented_bbox(bbox, out_filename, colors)
    ...
    if len(scene_bbox) > 0:
        ...
        if single:
            for box, color in zip(scene_bbox, colors):
                write_oriented_bbox_single(box, out_filename, color)
        else:
            for box, color in zip(scene_bbox, colors):
...
def write_oriented_bbox_single(box, out_filename, color=None):
    scene = trimesh.scene.Scene()
    if color is None:
    ...
    mesh_list = trimesh.util.concatenate(scene.dump())
    import random
    mesh_list.export(out_filename + f"{''.join(random.sample('qwertyuiopasdfghjklzxcvbnm', 8))}.ply", file_type='ply')
